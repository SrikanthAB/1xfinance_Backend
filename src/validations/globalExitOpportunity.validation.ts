...
const GlobalExitOpportunityValidationSchema = z.object({
...
const CreateGlobalExitOpportunityValidation = GlobalExitOpportunityValidationSchema;
const UpdateGlobalExitOpportunityValidation = GlobalExitOpportunityValidationSchema.partial().refine(
  (data) => Object.keys(data).length > 0,
  { message: 'At least one field must be provided for update', path: [] }
);

export { CreateGlobalExitOpportunityValidation, UpdateGlobalExitOpportunityValidation }; 