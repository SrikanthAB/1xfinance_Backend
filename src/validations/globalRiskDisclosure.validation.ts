...
const GlobalRiskDisclosureValidationSchema = z.object({
...
const CreateGlobalRiskDisclosureValidation = GlobalRiskDisclosureValidationSchema;
const UpdateGlobalRiskDisclosureValidation = GlobalRiskDisclosureValidationSchema.partial().refine(
  (data) => Object.keys(data).length > 0,
  { message: 'At least one field must be provided for update', path: [] }
);

export { CreateGlobalRiskDisclosureValidation, UpdateGlobalRiskDisclosureValidation }; 